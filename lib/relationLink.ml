...
let someone_text conf base ip =
  ...
  referenced_person_title_text conf base p ^ Date_text.short_dates_text conf base p
    ...
    (ips, _) :: _, (Some "on", _ | _, Some "spouse") ->
      ...
      begin match get_parents a with
        Some ifam ->
          ...
          in
          let d =
            Date_text.short_marriage_date_text conf base fam
      ...
      with
        Some ifam1, Some ifam2 ->
          ...
          begin match other_parent with
            Some ip ->
              let d =
                Date_text.short_marriage_date_text conf base (foi base ifam1)
